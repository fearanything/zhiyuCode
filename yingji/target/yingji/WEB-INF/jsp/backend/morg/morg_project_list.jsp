                            ...
                            <table style="margin-top:5px;">
                                <tr>
                                    ...
                                    <td style="vertical-align:top;padding-left:2px;">
                                        <select class="chosen-select form-control" name="PROJECT_ORG" id="PROJECT_ORG" data-placeholder="所属单位" style="vertical-align:top;width: 200px;">
                                            <option value=""></option>
                                            <option value="">全部</option>
                                            <c:forEach items="${unitList}" var="map" varStatus="vs">
                                                <option value="${map.ORG_ID}" <c:if test="${map.ORG_ID == pd.PROJECT_ORG}">selected</c:if> >${map.ORG_NAME}</option>
                                            </c:forEach>
                                        </select>
                                    </td>
                                    ...
                                    <c:when test="${not empty varList}">
                                        ...
                                        <c:forEach items="${varList}" var="var" varStatus="vs">
                                            <tr>
                                                ...
                                                <td class='center'><a href="javascript:goSondictProject('${var.ORG_ID}')"><i class="ace-icon fa fa-share bigger-100"></i>&nbsp;${var.ORG_NAME}</a></td>
                                                <td class='center'>${var.ORG_NAME_SHORT}</td>
<%--                                                <td class='center'>${var.ORG_LEVEL}</td>--%>
                                                <td class='center'>
                                                    <c:choose>
                                                        <c:when test="${fn:length(var.REMARK) > 30}">
                                                            <c:out value="${fn:substring(var.REMARK, 0, 30)}......" />
                                                        </c:when>
                                                        <c:otherwise>
                                                            <c:out value="${var.REMARK}" />
                                                        </c:otherwise>
                                                    </c:choose>
                                                </td>
                                    ...
                                    <tr>
                                        <td style="vertical-align:top;">
                                            ...
                                            <c:if test="${null != pd.ORG_ID && pd.ORG_ID != ''}">
                                                <a class="btn btn-mini btn-success" onclick="goSondictProject('${pd.PARENT_ID}');">返回</a>
                                            </c:if>
                                        </td>
...
<script type="text/javascript">
    ...
    //去此ID下子级列表
    function goSondictProject(ORG_ID,ISORG){
        console.log("返回上级机构")
        top.jzts();
        window.location.href="<%=basePath%>morg/list.do?ORG_ID="+ORG_ID+'&ISORG=0';
    };
    ...
    function toExcel(){
        ...
        window.location.href='<%=basePath%>morg/excelProject?'+$.param(param);
    }
    ...
    function fromExcel(){
        ...
        diag.URL = '<%=basePath%>morg/goUploadExcelProject.do';
        diag.Width = 300;
        diag.Height = 150;
        diag.CancelEvent = function(){ //关闭事件
            if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
