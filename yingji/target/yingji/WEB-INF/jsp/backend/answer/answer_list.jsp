						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center" style="width:10%;">应急响应级别</th>
									<th class="center" style="width:25%;">应急响应单位</th>
									<th class="center" style="width:15%;">开始时间</th>
									<th class="center" style="width:15%;">结束时间</th>
									<th class="center" style="width:20%;">应急响应备注</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.ANSWER_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>
											 ...
											 <c:if test="${data.key == var.ANSWER_LEVEL }">${data.value}</c:if>
										     </c:forEach>
											</td>
											<td class='center'>${var.ANSWER_COMPANY}</td>
											<td class='center'>${var.START_DATETIME}</td>
											<td class='center'>${var.END_DATETIME}</td>
											<td class='center'>${var.ANSWER_REMARK}</td>
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>answer/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
