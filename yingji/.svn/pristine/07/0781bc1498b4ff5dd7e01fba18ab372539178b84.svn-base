										...
										<c:if test="${QX.cha == 1 }">
											<c:forEach items="${userList}" var="user" varStatus="vs">
												...
												<tr>
													...
													<td class="center">${user.USERNUM }</td>
													...
													<td class="center">
														...
														<div class="hidden-sm hidden-xs btn-group">
															...
															<c:if test="${QX.edit == 1 }">
																<a class="btn btn-xs btn-success" title="展示页面设置" onclick="userOrgShow('${user.USER_ID}');">
																	展示页面设置
																</a>
															</c:if>
															<c:if test="${QX.edit == 1 }">
																<a class="btn btn-xs btn-success" title="组织权限" onclick="editRights('${user.USER_ID}');">
																	组织权限
																</a>
															</c:if>
															<c:if test="${QX.edit == 1 }">
...
<script type="text/javascript">
	...
	//组织权限
	function userOrgShow(USER_ID){
		top.jzts();
		var diag = new top.Dialog();
		diag.Drag = true;
		diag.Title = "组织权限";
		diag.URL = '<%=basePath%>user/userOrgShow.do?USER_ID='+USER_ID;
		diag.Width = 400;
		diag.Height = 450;
		diag.CancelEvent = function(){ //关闭事件
			diag.close();
	...
	function editRights(USER_ID){
		...
		diag.Drag = true;
		diag.Title = "组织权限";
		diag.URL = '<%=basePath%>user/orgqx.do?USER_ID='+USER_ID;
		diag.Width = 320;
		diag.Height = 450;
		diag.CancelEvent = function(){ //关闭事件
			diag.close();
