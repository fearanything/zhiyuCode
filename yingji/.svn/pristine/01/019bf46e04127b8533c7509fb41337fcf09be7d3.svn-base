...
public class LoginController extends BaseController {
	...
	public Object login()throws Exception{
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){//判断效验码
				...
				logger.info("缓存的验证码：" + sessionCode);
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu){
		...
		try{
			...
			if (user != null) {
				...
				session.setAttribute(Const.SESSION_USERID, USER_ID);		//放入用户ID到session
	...
	public ModelAndView defaultPage() throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		if(null!=userService.getUserCount("").get("userCount")) {
			pd.put("userCount", Integer.parseInt(userService.getUserCount("").get("userCount").toString())-1);				//系统用户数
		}
		if(null!=appuserService.getAppUserCount("").get("appUserCount")) {
