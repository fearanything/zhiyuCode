...
public class WaterController extends BaseController {
	...
	String menuUrl = "water/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Water");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("WATER_ID", this.get32UUID());	//主键
		pd.put("SORT", "0");	//排序
		pd.put("ISDEL", "0");	//删除标志
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Water");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		waterService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Water");
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Water");
		...
		page.setPd(pd);
		List<PageData>	varList = waterService.list(page);	//列出Water列表
		mv.setViewName("backend/water/water_list");
		...
		mv.addObject("reservoirMap", reservoirList);
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("backend/water/water_edit");
		List<PageData> typhoonList=typhoonService.listAll(pd);
		mv.addObject("typhoonMap", typhoonList);
		List<PageData> reservoirList=reservoirService.listAll(pd);
		mv.addObject("reservoirMap", reservoirList);
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = waterService.findById(pd);	//根据ID读取
		mv.setViewName("backend/water/water_edit");
		List<PageData> typhoonList=typhoonService.listAll(pd);
		mv.addObject("typhoonMap", typhoonList);
		List<PageData> reservoirList=reservoirService.listAll(pd);
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Water");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			waterService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Water到excel");
		...
		List<PageData> varOList = waterService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("WATER_ID"));	    //1
			vpd.put("var2", varOList.get(i).getString("TYPHOON_ID"));	    //2
			vpd.put("var3", varOList.get(i).getString("RESERVOIR_ID"));	    //3
			vpd.put("var4", varOList.get(i).getString("WATER_DATETIME"));	    //4
			vpd.put("var5", varOList.get(i).get("WATER_DATA").toString());	//5
