...
public class RiskController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Risk");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		pd.put("RISK_ID", this.get32UUID());	//主键
		pd.put("SORT", "0");	//排序
		pd.put("ISDEL", "0");	//删除标志
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Risk");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		riskService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView list(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = riskService.list(page);	//列出Risk列表
		mv.setViewName("backend/risk/risk_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("backend/risk/risk_edit");
		List<PageData> typhoonList=typhoonService.listAll(pd);
		mv.addObject("typhoonMap", typhoonList);
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = riskService.findById(pd);	//根据ID读取
		mv.setViewName("backend/risk/risk_edit");
		List<PageData> typhoonList=typhoonService.listAll(pd);
		mv.addObject("typhoonMap", typhoonList);
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Risk");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			riskService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Risk到excel");
		...
		List<PageData> varOList = riskService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("RISK_ID"));	    //1
			vpd.put("var2", varOList.get(i).getString("TYPHOON_ID"));	    //2
			vpd.put("var3", varOList.get(i).getString("RISK_NAME"));	    //3
			vpd.put("var4", varOList.get(i).getString("RISK_INFO"));	    //4
