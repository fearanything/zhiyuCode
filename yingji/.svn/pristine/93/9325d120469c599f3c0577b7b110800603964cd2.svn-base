...
<%--						<input type="hidden" name="ORG_ID" id="ORG_ID" value="${pd.ORG_ID}"/>--%>
						...
						<input type="hidden" name="RECTIFY_STAGE" id="RECTIFY_STAGE" value="${pd.RECTIFY_STAGE}"/>
						<input type="hidden" name="RECTIFYINFO_AREA" id="RECTIFYINFO_AREA" value="${pd.RECTIFYINFO_AREA}"/>
...
<%--								</td>--%>
							...
							<tr>
								...
								<td>
									<select class="chosen-select form-control" name="PROJECT_NAME" id="PROJECT_NAME" data-placeholder="这里输入项目名称" style="vertical-align:top;width: 98%;">
										<option value=""  > </option>
										<c:forEach items="${projectList}" var="map" varStatus="vs">
											<option value="${map.ORG_ID}" <c:if test="${map.ORG_ID == pd.PROJECT_NAME}">selected</c:if> >${map.ORG_NAME}</option>
										</c:forEach>
									</select>
							</tr>
							<tr>
								...
								<td colspan="10">
									<div>
										<ul class="ace-thumbnails clearfix" id="imgList">
											...
											<c:choose>
												<c:when test="${not empty pd.IMG_ARR}">
													<c:forEach items="${pd.IMG_ARR}" var="img" varStatus="vs">
														<li>
															<a href="${img}" data-rel="colorbox" class="cboxElement">
															...
															<div class="tools tools-bottom">
																<a href="javascript:;" onclick="delImg('${img}', this);">
							...
							<tr>
								<td style="text-align: right;padding-top: 13px;"><span style="color: red;">*</span>整改期限:</td>
								<td>
									<input class="span10 date-picker" name="PLAN_COMPLETE_TIME" id="PLAN_COMPLETE_TIME" value="${pd.PLAN_COMPLETE_TIME}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" style="width:98%;" placeholder="整改期限" title="计划完成时间"/>
								</td>
							</tr>
							<tr>
								<td style="text-align: right;padding-top: 13px;"><span style="color: red;">*</span>所属区域:</td>
								<td>
									<div class="selectTree" id="selectTree"></div>
<%--									<select class="chosen-select form-control" name="RECTIFYINFO_AREA" id="RECTIFYINFO_AREA" data-placeholder="所属区域" style="vertical-align:top;width: 98%;">--%>
...
<%--									</select>--%>
							...
							<tr>
								<td style="text-align: right;padding-top: 13px;">填报人:</td>
								<td><input type="text" name="REPORTING_PERSON_FIRST" id="REPORTING_PERSON_FIRST" value="${pd.REPORTING_PERSON_FIRST}" maxlength="100" placeholder="这里输入填报人" title="填报人" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="text-align: right;padding-top: 13px;">填报单位:</td>
								<td>
									<select class="chosen-select form-control" name="REPORTING_UNIT_FIRST" id="REPORTING_UNIT_FIRST" data-placeholder="填报单位" style="vertical-align:top;width: 98%;">
										<option value=""></option>
										<c:forEach items="${orgList}" var="map" varStatus="vs">
											<option value="${map.ORG_ID}" <c:if test="${map.ORG_ID == pd.REPORTING_UNIT_FIRST}">selected</c:if> >${map.ORG_NAME}</option>
										</c:forEach>
									</select>
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<select class="chosen-select form-control" name="RESPONSIBLE_UNIT" id="RESPONSIBLE_UNIT" data-placeholder="责任单位" style="vertical-align:top;width: 98%;">
										<c:forEach items="${orgList}" var="map" varStatus="vs">
											<option value="${map.ORG_ID}" <c:if test="${map.ORG_ID == pd.RESPONSIBLE_UNIT}">selected</c:if> >${map.ORG_NAME}</option>
										</c:forEach>
									</select>
								</td>
							</tr>
							...
							<tr>
								...
								<td>
									<input type="text" name="REPORTING_PERSON" id="REPORTING_PERSON" value="${pd.REPORTING_PERSON}" maxlength="100" placeholder="这里输入填报人" title="整改投入填报人" style="width:98%;"/>
								</td>
							</tr>
							<tr>
			...
			$("#selectTree").bind("change",function(){
				if(!$(this).attr("relValue")){
			      //  top.Dialog.alert("没有选择节点");
					...
					//加入org判断
					$("#RECTIFYINFO_AREA").val($(this).attr("relValue"));
			    }
			});
			//赋给data属性
