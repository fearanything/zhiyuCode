...
<script src="<%=basePath%>frontend/hk/jsencrypt.min.js"></script>
<script src="<%=basePath%>frontend/hk/web-control_1.2.5.min.js"></script>
<script type="text/javascript">
   ...
   // 步骤1：监听父页面的消息
		...
		if(e && e.data){
			switch (e.data.action){
				...
				case 'updatePos':              // 更新插件位置：JS_CreateWnd时需要父页面计算滚动条偏移量，初始偏移量叠加该偏移量作为iframe的偏移量，防止插件窗口与DIV窗口初始不贴合情况
					...
					var X=$("#iframe1",window.parent.parent.document).offset().top;
					var Y=$("#iframe1",window.parent.parent.document).offset().left;
					oWebControl.JS_SetDocOffset({
						left: Y-50,
						top: X-25
					});  // 更新插件窗口位置
					
					oWebControl.JS_Resize(620, 500);
					setWndCover();	
					break;
	...
	function setWndCover() {
		if (oWebControl){
			...
			if (W2 < divWidth){
				...
				if (deltaRight > 0) {			
					oWebControl.JS_CuttingPartWindow(200 - deltaRight, 0, deltaRight + 1, 401);
				}
			}		
			...
			if (deltaBottom > 0) {
				oWebControl.JS_CuttingPartWindow(0, 90 - deltaBottom, 801, deltaBottom + 1);
			}
		}		
    }
		...
		oWebControl = new WebControl({
			...
			cbConnectSuccess: function () {
				...
				oWebControl.JS_StartService("window", {
					...
					oWebControl.JS_CreateWnd("playWnd", 200, 90, {
					    bEmbed: true,
    ...
    function cbIntegrationCallBack(oData) {
    	var result=oData.responseMsg.msg.result;
    	if(result=="816"){
    		...
    		$("#otherbox1",window.parent.parent.document).hide();
    	}
    ...
    function initBtnClicked(){
		...
		var param ='{'+
			    ...
			    '{'+
			    '    "appkey": "28247187",'+
			     '   "ip": "153.0.150.42:4430",'+
			     '   "port": 443,'+
			     '   "secret": "HQF64zbP6IEBTOVd1MP2",'+
			     '   "enableHTTPS": 1,'+
			      '  "layout": "1x1",'+
			     '   "playMode": 0,'+
			     '   "showToolbar": 1,'+
