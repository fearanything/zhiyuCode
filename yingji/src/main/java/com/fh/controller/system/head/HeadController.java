...
public class HeadController extends BaseController {
	...
	public Object getList() {
		...
		try {
			...
			PageData pdFhsms= fhsmsService.findFhsmsCount(Jurisdiction.getUsername());
			if(null!=pdFhsms&&null!=pdFhsms.get("fhsmsCount")) {
				map.put("fhsmsCount", pdFhsms.get("fhsmsCount").toString());//站内信未读总数	
			}else {
				map.put("fhsmsCount", "0");//站内信未读总数	
			}
	...
	public ModelAndView saveSys4() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.LOGINEDIT,pd.getString("isZhuce")+",fh,"+pd.getString("isMusic")+",fh,"+pd.getString("isApprove"));	//登录页面配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
