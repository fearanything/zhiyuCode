...
public class Page {
	...
	//拼接分页 页面及JS函数；用于PC端分页显示
	public String getPageStrPC() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("<div id=\"pagec\"> \n");
			if(currentPage==1){
				sb.append("");
			}else{
				sb.append("<div onclick=\"nextPage("+(currentPage-1)+")\" ><<</div>\n");
			}
			...
			for(int i=startTag; i<=totalPage && i<=endTag; i++){
				if(currentPage==i)
					sb.append("<div class=\"active\">"+i+"</div>\n");
				else
					sb.append("<div onclick=\"nextPage("+i+")\">"+i+"</div>\n");
			}
			if(currentPage==totalPage){
				sb.append("");
			}else{
	...
	public void setPageStrZkzx(String pageStrZkzx) {
		PageStrZkzx = pageStrZkzx;
	}
	 ...
	 * 用于智库在线的分页拼接
	 * */
	public String getPageStrZkzx() {
		StringBuffer sb = new StringBuffer();
		if(totalResult>0){
			sb.append("	<div class='page-prev dis_mid'>");
			if(currentPage==1){
				sb.append("	<a>上页</a>\n");
			}else{
				sb.append("	<a onclick=\"nextPage("+(currentPage-1)+",'"+dataId+"','"+callIndex+"')\">上页</a>\n");
			}
			sb.append("	</div>\n");
			sb.append("	<div class='page-num dis_mid clearfix'>\n");
			int showTag = 9;//分页标签显示数量
			int startTag = 1;
			if(currentPage>showTag){
				startTag = currentPage-1;
			}
			...
			for(int i=startTag; i<=totalPage && i<=endTag; i++){
				if(currentPage==i)
					...
					sb.append("<a onclick=\"nextPage("+i+",'"+dataId+"','"+callIndex+"')\">"+i+"</a>\n");
			}
			sb.append("</div>\n");
			sb.append("<div class=\"page-next dis_mid\">\n");
			if(currentPage==totalPage){
				sb.append("	<a>下页</a>\n");
			}else{
				sb.append("	<a onclick=\"nextPage("+(currentPage+1)+",'"+dataId+"','"+callIndex+"')\">下页</a>\n");
			}
...
sb.append("<script type=\"text/javascript\">\n");
			...
			//换页函数
			sb.append("function nextPage(page,id,callIndex){");
			sb.append(" top.jzts();");
			sb.append("	if(true && document.getElementById(\"Form_\"+id)){\n");
			sb.append("		var url = document.getElementById(\"Form_\"+id).getAttribute(\"action\");\n");
			sb.append("		if(url.indexOf('?')>-1){url += \"&"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
			sb.append("		else{url += \"?"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
			sb.append("		url = url + page + \"&" +(entityOrField?"showCount":"page.showCount")+"="+showCount+"\";\n");
			sb.append("		document.getElementById(\"Form_\"+id).action = url+\"&CALL_INDEX=\"+callIndex+\"&CATEGORY_ID=\"+id;\n");
			sb.append("		document.getElementById(\"Form_\"+id).submit();\n");
			sb.append("	}else{\n");
			sb.append("		var url = document.location+'';\n");
			sb.append("		if(url.indexOf('?')>-1){\n");
			sb.append("			if(url.indexOf('currentPage')>-1){\n");
			...
			sb.append("		}else{url += \"?"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
			sb.append("		url = url + page + \"&" +(entityOrField?"showCount":"page.showCount")+"="+showCount+"\";\n");
			sb.append("		document.location = url+\"&CALL_INDEX=\"+callIndex+\"&CATEGORY_ID=\"+id;\n");
			sb.append("	}\n");
