...
<%--                    </c:if>--%>
<%--                      <c:if test="${QX.add == 1}">--%>
<%--                        <a class="btn btn-xs btn-success" onclick="makeAllSubmit('确定要提交选中的数据吗?');" title="批量提交" >批量提交</a>--%>
<%--                      </c:if>--%>
<%--                  </td>--%>
<%--                </tr>--%>
<%--              </table>--%>
...
<script type="text/javascript">
  ...
  function edit(Id){
    ...
    diag.Drag=true;
    diag.Title ="编辑";
    diag.URL = '<%=basePath%>rectifyinfo/goEditNoSubmit.do?RECTIFY_ID='+Id;
    diag.Width = 900;
    diag.Height = 800;
    diag.Modal = true;				//有无遮罩窗口
    diag. ShowMaxButton = true;	//最大化按钮
      ...
      if(result) {
        ...
        if(str==''){
          ...
          if(msg == '确定要提交选中的数据吗?'){
            ...
            $.ajax({
              type: "POST",
              url: '<%=basePath%>rectifyinfo/submitAll.do?tm='+new Date().getTime(),
              data: {DATA_IDS:str},
              dataType:'json',
              //beforeSend: validateData,
              cache: false,
